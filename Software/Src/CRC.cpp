...
/**
 * @brief CRC8
 * @note CRC8 generator polynomial:G(x)=x8+x5+x4+1
 */
...
uint16_t Get_CRC16(uint8_t *Data, uint32_t Length) {
	...
	while (Length--) (wCRC) = ((uint16_t)(wCRC) >> 8) ^ CRC16_TAB[((uint16_t)(wCRC) ^ (uint16_t)(*Data++)) & 0x00FF];
	return wCRC;
}
